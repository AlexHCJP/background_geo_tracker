...
public class BackgroundGeoTrackerPlugin: NSObject, FlutterPlugin {
    private let config = GeoConfigStore()

    public static func register(with registrar: FlutterPluginRegistrar) {
        let instance = BackgroundGeoTrackerPlugin()

        let methods = FlutterMethodChannel(
            name: "background_geo_tracker/geo",
            binaryMessenger: registrar.messenger()
        )
        registrar.addMethodCallDelegate(instance, channel: methods)

        FlutterEventChannel(
            name: "background_geo_tracker/geo/points",
            binaryMessenger: registrar.messenger()
        ).setStreamHandler(
            GeoStreamHandler(
        ...
        FlutterEventChannel(
            name: "background_geo_tracker/geo/status",
            binaryMessenger: registrar.messenger()
        ).setStreamHandler(
            GeoStreamHandler(
        ...
        BackgroundGeoTrackerLaunch.wireUploader()
        _ = BackgroundGeoTrackerLaunch.resumeIfTracking()
    }
    ...
    /// `BackgroundGeoTrackerLaunch.resumeIfTracking()`, which the host calls from its
